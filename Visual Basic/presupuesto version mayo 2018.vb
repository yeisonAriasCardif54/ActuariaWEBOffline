...
For i = 1 To totalproduct
   For k = 1 To meses
       ...
       If k > k1 Then
          ...
          Cells((k - k1) + 1 + (meses - k1) * j, 27) = ResulTec(i, k) 'RT
...
Sub imprimir(j, k, k1, meses, id, socio, cod, oferta, tprima, tproyec, grupo, fecha, nuevo, vig, cancel, sinis, gwp, upr, ep, comin, comins, cominb, dac, ec, ecs, ecb, ic, _
    rt, vatp, vata, incent, incentp, tmk, ica, gmf, vati, vatmk, gros2, over, goi, ln, cj, capa, nombre, mp, ap, ngrupo, gwpn, reqc, reqca, fincon)
       ...
       Cells((k - k1) + 1 + (meses - k1) * j, 27) = rt 'Resultado Tecnico
       Cells((k - k1) + 1 + (meses - k1) * j, 28) = vatp 'IVA no descontable pagado
       Cells((k - k1) + 1 + (meses - k1) * j, 29) = vata 'IVA no descontable amortizado
       ...
       'Cells((k - k1) + 1 + (meses - k1) * j, 41) = WorksheetFunction.Max(goi - tx, 0) 'NOI
       Cells((k - k1) + 1 + (meses - k1) * j, 41) = ln 'Linea negocio socio
       Cells((k - k1) + 1 + (meses - k1) * j, 42) = cj 'Oferta CJ
       Cells((k - k1) + 1 + (meses - k1) * j, 43) = capa 'Capa
       Cells((k - k1) + 1 + (meses - k1) * j, 44) = nombre 'Nombre del producto
       Cells((k - k1) + 1 + (meses - k1) * j, 45) = mp 'Mes de proyeccìon
       Cells((k - k1) + 1 + (meses - k1) * j, 46) = ap 'Año de proyección
