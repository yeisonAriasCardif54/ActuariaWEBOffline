...
ReDim comision2(1 To totalproduct) 'Valor de comision 2
ReDim impuestoReaseguro(1 To totalproduct) 'Valor de comision 2
...
ReDim impReaseguro(1 To meses + mesiniciost) 'Calculo de Impuesto reaseguro
...
ReDim commin2(1 To meses + mesiniciost) 'Comisiones 2
ReDim dac(1 To meses + mesiniciost)
ReDim dac1(1 To meses + mesiniciost)
ReDim dac2(1 To meses + mesiniciost)
...
ReDim earnedC2(1 To totalproduct, 1 To meses + mesiniciost)
...
ReDim impReasegurog(1 To meses + mesiniciost, 1 To ngrupos) 'Impuesto Reaseguro grupal
...
Sheets("ParametrosSt").Activate
  ...
  Next j
	...
	comision2(i) = Cells(i + 1, j + 7)
	impuestoReaseguro(i) = Cells(i + 1, j + 8)
...
Sheets("ParametrosNv").Activate
  ...
  Next j
	...
	comision2(i) = Cells(i + 1 - sproduct, j + 12)
	impuestoReaseguro(i) = Cells(i + 1 - sproduct, j + 13)
    j = 1
     ...
     'GWP, primas emitidas: calculo por tipo de prima -----------------------------------------------------------------------------------------------------------------------------------------
           ...
           If infproducto(5, 1) = 1 Then 'si la prima resulta ser mensual = 1 --------------------------------------------------------------------------------------------------------------
               ...
               Else
                  ...
                  gwpn(k) = 0
				  'IMPUESTO REASEGURO
				  impReaseguro(k) = gwp(k) * impuestoReaseguro(i)
               End If
 ...
 '------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
           'Obtiene las comisiones
            ...
            Else
			  ...
			  commin2(k) = gwp(k) * comision2(i)
            End If

           'Obtiene las comisiones diferidas
           dac(k) = upr(k) *  (infproducto(10, 1) + comision2(i))  ' upr(k) * commin(k)

		   dac1(k) = upr(k) *  infproducto(10, 1)  ' upr(k) * commin(k)
		   dac2(k) = upr(k) *  comision2(i)  ' upr(k) * commin(k)

           'Obtiene las comisiones devengadas
           If k = 1 Then
             earnedC(i, k) = commin(k) - dac1(k)  ' commin(k) - dac(k)
			 earnedC2(i, k) = commin2(k) - dac2(k)
           Else
             earnedC(i, k) = commin(k) - dac1(k) + dac1(k - 1) ' commin(k) - dac(k) + dac(k - 1)
			 earnedC2(i, k) = commin2(k) - dac2(k) + dac2(k - 1)
           End If

           'Iva no descontable
           vatp(k) = infproducto(14, 1) * infproducto(15, 1) * (commin(k) + commin2(k)) * piva ' infproducto(14, 1) * infproducto(15, 1) * commin(k) * piva
           vata(k) = infproducto(14, 1) * infproducto(15, 1) * (earnedC(i, k) + earnedC2(i, k)) * piva
        ...
        'GWP: primas emitidas: calculo por tipo de prima ----------------------------------------------------------------------------------------------------------------------------------------
           ...
           If infproducto(5, 1) = 1 Then 'si la prima resulta ser mensual = 1 --------------------------------------------------------------------------------------------------------------
               ...
               End If
				  ...
				  'IMPUESTO REASEGURO
				  impReaseguro(k + mesiniciost) = gwp(k + mesiniciost) * impuestoReaseguro(i + mesiniciost)
           ...
           End If 'Termina el condicional de tipo de prima segunda vez
            ...
            If k = tcutoff And tcutoff > 0 Then
               commin(k + mesiniciost) = -dac(k + mesiniciost - 1)
			   commin2(k + mesiniciost) = -dac2(k + mesiniciost - 1)
            Else
               commin(k + mesiniciost) = gwp(k + mesiniciost) * infproducto(10, 1)
			   commin2(k + mesiniciost) = gwp(k + mesiniciost) * comision2(i)
            End If

           'Obtiene las comisiones diferidas
           dac1(k + mesiniciost) = upr1(k + mesiniciost) * infproducto(10, 1)
		   dac2(k + mesiniciost) = upr1(k + mesiniciost) * comision2(i)


           'Obtiene las comisiones devengadas de las renovaciones
           If k = 1 Then
             earnedC(i, k + mesiniciost) = commin(k + mesiniciost) - dac1(k + mesiniciost)
			 earnedC2(i, k + mesiniciost) = commin2(k + mesiniciost) - dac2(k + mesiniciost)
           Else
             earnedC(i, k + mesiniciost) = commin(k + mesiniciost) - dac1(k + mesiniciost) + dac1(k + mesiniciost - 1)
			 earnedC2(i, k + mesiniciost) = commin2(k + mesiniciost) - dac2(k + mesiniciost) + dac2(k + mesiniciost - 1)
           End If

           'Iva no descontable
           vatp(k + mesiniciost) = infproducto(14, 1) * infproducto(15, 1) * (commin(k + mesiniciost) + commin2(k + mesiniciost)) * piva
           vata(k + mesiniciost) = infproducto(14, 1) * infproducto(15, 1) * (earnedC(i, k + mesiniciost) + earnedC2(i, k + mesiniciost)) * piva
...
4000 'ìndice para encontrar el procedimiento de suma de variables
    ...
    'Variables para el agrupamiento por PU----------------------------------------------------------------------------------------------------------------------------------------------------------
		   ...
		   impReasegurog(k + mesiniciost, pugroup(i)) = impReasegurog(k + mesiniciost, pugroup(i)) + impReaseguro(k + mesiniciost)
...
Call imprimir(ji, k, k1, meses, infproducto(1, 1), infsocio(infproducto(2, 1)), infproducto(18, 1), infoferta(infproducto(3, 1)), inftipoprima(infproducto(5, 1)), infproducto(19, 1), pugroup(i), _
     fechas(k), nuevos(k), vigentes(k), cancela(k), sinies(k), gwp(k), upr(k), earnedP(i, k), commin(k), (commin(k) + commin2(k)) * particomin(1), (commin(k) + commin2(k)) * particomin(2), _
     dac(k), earnedC(i, k), (earnedC(i, k) + earnedC2(i, k)) * particomin(1), (earnedC(i, k) + earnedC2(i, k)) * particomin(2), incurC(i, k), ResulTec(i, k), vatp(k), vata(k), _
     ...
     gwpn(k), reqcap(k), reqcapy(k), (fincomec(k) + fincomer(k)), commin2(k), earnedC2(i, k), impReaseguro(k))
Else
GoTo 8000
End If
8000
...
'Decidir cual incentivo usar
       ...
       gross1(k) = earnedP(i, k) - (earnedC(i, k) + earnedC2(i, k)) - vata(k) - pagincentivos2 - tmkCost(k) - incurC(i, k) - gastos(i, k) - vatincent(k) - vatmk(k) - impReaseguro(k)
...
For i = 1 To ngrupos
   ...
   For k = 1 To meses
    ...
    goi1(k, i) = earnedPreg(k, i) - earnedCog(k, i) - incurCg(k, i) - VAT_Amortig(k, i) - incentig(k, i) - tmktCostg(k, i) - overheadg(k, i) - icag(k, i) - gmfg(k, i) - vatincentg(k, i) - vatmkg(k, i) - impReasegurog(k, i)
...
For i = 1 To totalproduct
   For k = 1 To meses
     If purealg(k, pugroup(i)) > 0 Then
       ResulTec(i, k) = earnedP(i, k) - (earnedC(i, k) + earnedC2(i, k)) - incurC(i, k) 'Resultado tecnico prima
...
For i = 1 To totalproduct
   For k = 1 To meses
       ...
       ResulTec(i, k) = earnedP(i, k) - (earnedC(i, k) + earnedC2(i, k)) - incurC(i, k) - puincur(i, k)
       ...
       If k > k1 Then
          ...
          Cells((k - k1) + 1 + (meses - k1) * j, 27) = pureal(i, k) 'PU Real
          Cells((k - k1) + 1 + (meses - k1) * j, 28) = puincur(i, k) 'PU Incurrida
          Cells((k - k1) + 1 + (meses - k1) * j, 29) = ResulTec(i, k) 'RT
          Cells((k - k1) + 1 + (meses - k1) * j, 39) = gross2(i, k) + gastos(i, k) - Cells((k - k1) + 1 + (meses - k1) * j, 53) 'NBI
          Cells((k - k1) + 1 + (meses - k1) * j, 42) = gross2(i, k) 'GOI
...
For i = 1 To totalproduct
   For k = 1 To meses
      ...
      Cells((k - k1) + 1 + (meses - k1) * j, 43) = taxreal(i, k) 'taxes
...
Sub imprimir(j, k, k1, meses, id, socio, cod, oferta, tprima, tproyec, grupo, fecha, nuevo, vig, cancel, sinis, gwp, upr, ep, comin, comins, cominb, dac, ec, ecs, ecb, ic, _
    rt, vatp, vata, incent, incentp, tmk, ica, gmf, vati, vatmk, gros2, over, goi, ln, cj, capa, nombre, mp, ap, ngrupo, gwpn, reqc, reqca, fincon, comin2, ec2, impReaseguro)
	   ...
	   Cells((k - k1) + 1 + (meses - k1) * j, 18) = comin2 'Total Commisions 2

       Cells((k - k1) + 1 + (meses - k1) * j, 19) = comins 'Comisiones de socio
       Cells((k - k1) + 1 + (meses - k1) * j, 20) = cominb 'Comisiones de broker
       Cells((k - k1) + 1 + (meses - k1) * j, 21) = dac 'Comisiones diferidas
       Cells((k - k1) + 1 + (meses - k1) * j, 22) = ec 'Comisiones devengadas

	   Cells((k - k1) + 1 + (meses - k1) * j, 23) = ec2 'Total Earned_Commissions 2

       Cells((k - k1) + 1 + (meses - k1) * j, 24) = ecs 'Comisiones devengadas de socio
       Cells((k - k1) + 1 + (meses - k1) * j, 25) = ecb 'Comisiones devengadas de broker
       Cells((k - k1) + 1 + (meses - k1) * j, 26) = ic 'Incurred Claims
       'Cells((k - k1) + 1 + (meses - k1) * j, 27) = pu 'PU_eop
       'Cells((k - k1) + 1 + (meses - k1) * j, 28) = pui 'PU incurrida del mes
       Cells((k - k1) + 1 + (meses - k1) * j, 29) = rt 'Resultado Tecnico
       Cells((k - k1) + 1 + (meses - k1) * j, 30) = vatp 'IVA no descontable pagado
       Cells((k - k1) + 1 + (meses - k1) * j, 31) = vata 'IVA no descontable amortizado
       Cells((k - k1) + 1 + (meses - k1) * j, 32) = incent 'Incentivos amortizado
       Cells((k - k1) + 1 + (meses - k1) * j, 33) = incentp 'Incentivo pagado
       Cells((k - k1) + 1 + (meses - k1) * j, 34) = tmk 'Costo de TMKT
       Cells((k - k1) + 1 + (meses - k1) * j, 35) = ica 'Impuesto del ICA
       Cells((k - k1) + 1 + (meses - k1) * j, 36) = gmf 'el 4x1000
       Cells((k - k1) + 1 + (meses - k1) * j, 37) = vati 'vat de incentivos
       Cells((k - k1) + 1 + (meses - k1) * j, 38) = vatmk 'Vat de tmk
       'Cells((k - k1) + 1 + (meses - k1) * j, 39) = gros2 'NBI
       Cells((k - k1) + 1 + (meses - k1) * j, 40) = over '% overheads*EP

	   Cells((k - k1) + 1 + (meses - k1) * j, 41) = impReaseguro 'Impuesto Reaseguro

       'Cells((k - k1) + 1 + (meses - k1) * j, 42) = goi 'GOI de cada producto
       'Cells((k - k1) + 1 + (meses - k1) * j, 43) = tx 'taxes
       Cells((k - k1) + 1 + (meses - k1) * j, 44) = ln 'Linea negocio socio
       Cells((k - k1) + 1 + (meses - k1) * j, 45) = cj 'Oferta CJ
       Cells((k - k1) + 1 + (meses - k1) * j, 46) = capa 'Capa
       Cells((k - k1) + 1 + (meses - k1) * j, 47) = nombre 'Nombre del producto
       Cells((k - k1) + 1 + (meses - k1) * j, 48) = mp 'Mes de proyeccìon
       Cells((k - k1) + 1 + (meses - k1) * j, 49) = ap 'Año de proyección
       Cells((k - k1) + 1 + (meses - k1) * j, 50) = ngrupo 'Nombre del grupo
       Cells((k - k1) + 1 + (meses - k1) * j, 51) = reqc 'Requerimiento de capital
       Cells((k - k1) + 1 + (meses - k1) * j, 52) = reqca 'Requerimiento de capital anual
       Cells((k - k1) + 1 + (meses - k1) * j, 53) = fincon 'Ingreso financiero
