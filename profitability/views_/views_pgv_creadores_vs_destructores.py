...
def pgv_graficar(request):
    ...
    for i, row in pgv2.iterrows():
        ...
        VALUE_CREATION_ACCUMULATED = VALUE_CREATION_ACCUMULATED + row['VALUE_CREATION']
        if row['VALUE_CREATION'] >= 0:
            color = '#20c997'
        else:
            color = '#f4516c'
        data.append({
            ...
            # "color": newColor[i],
            "color": color,
            "VALUE_CREATION_ACCUMULATED": VALUE_CREATION_ACCUMULATED,
            "minimo1": 5
        })

    data2 = []
    VALUE_CREATION_ACCUMULATED2 = 0
    for i, row in pgv2.iterrows():
        if row['VALUE_CREATION'] >= 0:
            VALUE_CREATION = '{:,}'.format(row['VALUE_CREATION'])
            PVGWP = '{:,}'.format(row['PVGWP'])
            PROFIT_MARGIN = '{:.2f}'.format(row['PROFIT_MARGIN']) + ' %'
            VALUE_CREATION_ACCUMULATED2 = VALUE_CREATION_ACCUMULATED2 + row['VALUE_CREATION']
            # VALUE_CREATION_ACCUMULATED2 = '{:,}'.format(VALUE_CREATION_ACCUMULATED2)
            data2.append({
                "label": str(row['LABEL']),
                "VALUE_CREATION": VALUE_CREATION,
                "PVGWP": PVGWP,
                "PROFIT_MARGIN": PROFIT_MARGIN,
                "VALUE_CREATION_ACCUMULATED": '{:,}'.format(VALUE_CREATION_ACCUMULATED2),
                "color": newColor[i],
            })

    pgv3 = pgv2.sort_values(by='VALUE_CREATION', ascending=True)
    print('\n\n\n - pgv3 - \n\n\n')
    print(pgv3)
    data3 = []
    VALUE_CREATION_ACCUMULATED3 = 0
    for i, row in pgv3.iterrows():
        if row['VALUE_CREATION'] < 0:
            VALUE_CREATION = '{:,}'.format(row['VALUE_CREATION'])
            PVGWP = '{:,}'.format(row['PVGWP'])
            PROFIT_MARGIN = '{:.2f}'.format(row['PROFIT_MARGIN']) + ' %'
            VALUE_CREATION_ACCUMULATED3 = VALUE_CREATION_ACCUMULATED3 + row['VALUE_CREATION']
            # VALUE_CREATION_ACCUMULATED3 = '{:,}'.format(VALUE_CREATION_ACCUMULATED3)
            data3.append({
                "label": str(row['LABEL']),
                "VALUE_CREATION": VALUE_CREATION,
                "PVGWP": PVGWP,
                "PROFIT_MARGIN": PROFIT_MARGIN,
                "VALUE_CREATION_ACCUMULATED": '{:,}'.format(VALUE_CREATION_ACCUMULATED3),
                "color": newColor[i],
            })

    cnxn3.close();
    return JsonResponse([data, data2, data3], safe=False)
...
def reporte(request):
    ...
    configurationView = {
        ...
        'last_name': last_name,
        'title': 'PGV - Creadores vs Destructores',
        'socios': socios,
