...
def index(request):
    ...
    configurationView = {
        ...
        'last_name': last_name,
        'title': 'Technical Analysis Note - Template',
        'area': 'Pricing',
        'herramienta': 'Tan',
        'file': 'pricing/tan.html',
    }
...
def generate(request):
    ...
    if request.method == 'POST' and request.FILES.get('file', False):
        ...
        if file_ext == 'xlsx':
            try:
                file = updateData(myFile)
                status = 1
                message = 'TAN generado con exito.'
            except:
                status = 0
                error = traceback.format_exc()
                message = 'ERROR:<br>Se presento un error: <br>' + error
        else:
            status = 0
    ...
    data = {"status": status, "message": message, 'file': file}
...
def updateData(myFile):
    ...
    df_01['Marca'] = np.where(((df_01.col1 == 'Valor Asegurado') | (df_01.col1 == 'Severidad') |
                               (df_01.col1 == 'frecuencias') | (df_01.col1 == 'Descuento') |
    ...
    def create_table(df, df_inp, inf, sup):
        ...
        df_tip_interm = (df_inp.loc[(df_inp['col1'] == 'Tipo de Intermediario'), 'col2'])
        ...
        # **************************************************************************
        # Data Edad suscripcion
        df_c = df[(df['col1'].isin(['Edad suscripcion', 'Edad permanencia']))]
        ...
        df_x = df[(df['col1'].isin(['Código Producto', 'Linea', 'Canal', 'Prima Neta de IVA',
                                    ...
                                    'Costo recaudo', 'Porcentaje de Gastos', 'Gastos', 'Comisión Banco', 'Comisión Intermediario', 'IVA_', 'Tipo de pago de prima']))]
        ...
        costo_eco = df_x.loc[(df_x['col1'] == 'Costo recaudo'), 'value']
        ...
        tipo_pago_prima = df_x.loc[(df_x['col1'] == 'Tipo de pago de prima'), 'value']
